...
if(("2F232EB71D584140B9529460340FCFE4" -eq $json.key) -and ("poweroff" -eq $inputArr[0].trim())) {
    ...
    if ($VMExists) {
        Stop-VM -VM $vmname -Confirm:$false -RunAsync 
        Write-Output "VM $vmname stopped successfully"
        Invoke-WebRequest $json.response_url -Method 'POST' -Headers @{'Content-Type' = 'application/json; charset=utf-8'} -Body "{'text': 'VM $vmname stopped successfully','response_type': 'in_channel'}"
    } else {
